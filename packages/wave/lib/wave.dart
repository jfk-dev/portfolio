...
class _WaveWidgetState extends State<WaveWidget> with TickerProviderStateMixin {
    ...
    if (widget.config.colorMode == ColorMode.custom) {
      ...
      if (!widget.isLoop) {
        _endAnimationTimer =
          ...
          for (final waveController in _waveControllers) {
            waveController.stop();
          }
        });
      }
  ...
  List<Widget> _buildPaints() {
    final paints = <Widget>[];
    if (widget.config.colorMode == ColorMode.custom) {
      final _colors = (widget.config as CustomConfig).colors;
      final _gradients = (widget.config as CustomConfig).gradients;
      final begin = (widget.config as CustomConfig).gradientBegin;
      final end = (widget.config as CustomConfig).gradientEnd;
...
class _CustomWavePainter extends CustomPainter {
  ...
  void _setPaths(double viewCenterY, Size size, Canvas canvas) {
    ...
    _layer.path!.lineTo(0, size.height);
    _layer.path!.close();
